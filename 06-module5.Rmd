...
If an equation is written entirely in terms of $t+1$ (e.g. $X^{t+1}= Y^{t+1}+Z^{t+1}$), it can be simplified to be written just in terms of $t$ because it's just referring to the same timestep (e.g. $X^{t}= Y^{t}+Z^{t}$). 
...
\end{equation}
\begin{equation}
R_p^{t+1}=\ S_{us}^{t^\prime}-\ S_{us(fc)}^{t}  ,   \text{if}\ \ \ S_{us}^{t^\prime}>\ S_{us(fc)}^{t}
(\#eq:module5-8)
\end{equation}
</center>
...
<center>
\begin{equation}
Q_{se}^t = \left\{\begin{matrix} {S}_{tot}^{\prime t}-\ S_{max}^t,\ \ &{S}_{tot}^{\prime t}>S_{max}
\\ 0,\ \ &{S}_{tot}^{\prime t}\le S_{max}
\end{matrix}\right.
(\#eq:module5-24)
\end{equation}
</center>
...
<center>
\begin{equation}
Q_{ie}^t = \left\{\begin{matrix} (P^t-I_{max})A_C,\ \ &{P}> I_{max}
\\ 0,\ \ &{P}\le I_{max}
\end{matrix}\right.
(\#eq:module5-25)
\end{equation}
