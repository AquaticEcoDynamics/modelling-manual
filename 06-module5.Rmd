...
As boundary conditions, we have a precipitation dataset, and a function (equation 21) to simulate potential evaporation (Figure \@ref(fig:module5-fig1)).
...
Start to write in the equations for storage, using the equations given in the table at the end of this document. Note that there are two variables that have a prime ($‘$) sign. These variables only exist theoretically in the model, so that we can use them for our IF statements.
Note that you may have to convert between storage volumes (m^3^) and heights (m) by multiplying or dividing by area (m^2^).
...
<center>
\begin{equation}
S_{us}^\prime=\ S_{us}^t+\ P^{t+1} A_{c} 
(\#eq:module5-7)
\end{equation}
\begin{equation}
R_p=\ S_{us}^\prime-\ S_{us(fc)}  ,   \text{if}\ \ \ S_{us}^\prime>\ S_{us(fc)}
(\#eq:module5-8)
\end{equation}
</center>
...
<center>
\begin{equation}
S_{sat}^{t+1}=\ S_{sat}^t+\ R_p^t-\ R_g^t-\ Q_{ss}^t-\ e_{bs(sat)}^t-\ e_{v(sat)}^t-\ e_{ag(sat)}^t
(\#eq:module5-9)
\end{equation}
</center>

(Note the use of “$t$” on the Right Hand Side for $R$, $Q$ and $e$ will be on the same row since they are computed based on yesterday’s $S$)
...
<center>
\begin{equation}
S_{gw}^{t+1}=\ S_{gw}^t+\ R_g^t-\ Q_{bf}^t\ 
(\#eq:module5-11)
\end{equation}
</center>
...
Total evapotranspiration of the system is the sum of $ET$ from deep rooted native vegetation, bare soil and agricultural crops in the catchment.
...
<center>
\begin{equation}
e_v= M (\frac{S_t}{S_{max}})^\varepsilon k_v e_p
(\#eq:module5-13)
\end{equation}
</center>
...
<center>
\begin{equation}
e_{ag(sat)}=\ \left[\frac{S_{sat}-\ \varphi D_{eag}}{S_{us}+\ (S_{sat}-\ \varphi D_{eag})}\right]e_{ag},\ \ if\ S_{sat}\geq\varphi D_{eag}
(\#eq:module5-18)
\end{equation}
</center>
...
<center>
\begin{equation}
e_{ag\left(us\right)}=\ e_{ag}-\ e_{ag\left(sat\right)}
(\#eq:module5-19)
\end{equation}
</center>
...
<center>
\begin{equation}
e_{bs} = \left\{\begin{matrix} (1-M-C)\alpha_{ebs}\left[\frac{S_{us}}{S_{max}-\ S_{sat}}\right]e_p,\ \ &S_{sat}<\varphi D_{ebs}
\\ (1-M-C)\alpha_{ebs}\left[\frac{S_{us}+\ \left(S_{sat}-\ \varphi D_{ebs}\right)}{S_{sat}-\ \varphi D_{ebs}}\right]e_p,\ \ &S_{sat}\geq\varphi D_{ebs}
\end{matrix}\right.
(\#eq:module5-20)
\end{equation}
</center>
...
<center>
\begin{equation}
e_{p(\text{daily})}=\ \frac{e_{p(\text{annual})}}{365}\left[1+A\ sin\left\{B\left(m-1\right)+d\right\}+C\right]\ 
(\#eq:module5-23)
\end{equation}
</center>
...
<center>
\begin{equation}
Q_{se} = \left\{\begin{matrix} {S\prime}_t-\ S_{max},\ \ &{S\prime}_t>S_{max}
\\ 0,\ \ &{S\prime}_t\le S_{max}
\end{matrix}\right.
(\#eq:module5-24)
\end{equation}
</center>
...
<center>
\begin{equation}
Q_{ss} = \left\{\begin{matrix} 0,\ \ &S_t\le S_{fc}
\\ \alpha\left(\frac{S_t}{S_{max}}\right)^\beta,\ \ &{S_{fc}\ <\ S}_t<S_{max}
\end{matrix}\right.
(\#eq:module5-26)
\end{equation}
</center>
...
For optional deep groundwater store:

<center>
\begin{equation}
Q_{bf}=\ \alpha_{bf}\left(S_{gw}\right)^{\beta_{bf}}
(\#eq:module5-27)
\end{equation}
</center>
