...
## Introduction to ggplot and the Tidyverse {-}
...
### Visualising the Flow Diversions {-}
...
- Pipes (`%>%`) take the output from one function make it the input of the next 
- `mutate()` is used to add or remove variables 
...
- Notice how `envFlow` is unchanged
...
- Group the data by `year` and calculate summary statistic with `summarise()`
...
- The data is 'binned', render it with `geom_bar()`
- `stat="identity"` sets the bar height to values in the data
