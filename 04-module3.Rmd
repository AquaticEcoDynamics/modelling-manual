...
There are no module resources needed for these exercises!
...
2) In Excel, construct a simple spreadsheet and fill in the blank cells of Table \@ref(tab:module3-1) to calculate the mean temperature of the Earth using the equation from Q1, assuming an albedo of 0.33 (including in °C and °F): 
...
kable(module3_1, format = "html", escape = F, align = "c", caption = "Fill in the blank cells to calculate the mean temperature of the Earth.") %>%
...
3) Test the sensitivity of this model to the choice of albedo. Using values from 0.3 to 0.7 in Table \@ref(tab:module3-2), calculate and plot T (°C)
...
kable(module3_2, format = "html", escape = F, align = "c", caption = "Test the sensitivity of this model.") %>%
  kable_styling(full_width = F, position = "center") %>%
...
kable(module3_3, format = "html", escape = F, align = "c", caption = "Estimate $T$ for a range of albedo values.") %>%
  kable_styling(full_width = F, position = "center") %>%
...
<center>
![](images/04-module3/picture3.png){width=90%}
</center>
...
kable(module3_4, format = "html", escape = F, align = "c", caption = "Energy balance model of Earth's atmosphere.") %>%
  ...
  scroll_box(width = "100%", height = "435px",
             fixed_thead = FALSE) 
```
</center>
<br>
...
<center>
\begin{equation}
k_{u}S + \sigma T_{l}^{4} + 0.5L = 2\sigma T_{u}^{4}
(\#eq:module3-7)
\end{equation}
</center>
<br>
...
1) Solve Equation \@ref(eq:module3-6) for $T_{u}$ in terms of $T_{s}$ and parameters.
2) Solve Equation \@ref(eq:module3-7) for $T_{l}$ in terms of $T_{u}$ and parameters.
...
4) Solve Equation \@ref(eq:module3-8) for $T_{s}$ in terms $T_{l}$, $T_{u}$, and parameters.
