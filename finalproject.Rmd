...
```{block2, projectideas, type='rmdtip2'}
Stuck for a starting point? Browse the [**Project Ideas dashboard**](ProjectIdeas/) — 73 topics across 10 themes, from coral bleaching to pit lakes to urban heat. Each one sets out the question, a suggested modelling approach, where to get the data, and a few references to read.

You do not have to take one of these. A topic of your own is always welcome — talk to the unit coordinator.
```
...
Every topic in the [Project Ideas dashboard](ProjectIdeas/) ends with a short reference list, including one paper chosen to show you *how to build the model* rather than to describe the system. That is a good place to see the level of detail expected here.
