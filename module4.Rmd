...
Remember back to Module 1 where we modelled salinity values from flow. Let's do something similar here to make a statistical relationship between year and CO~2~, Omega, and pH. Correlate the IPCC data of CO~2~/Year, pH/Year, and Omega/Year (i.e. use a scatterplot and add a trendline). Use these equations to generate modelled year values for our Exercise 3 model. Finally, you need to predict pH and Omega values from CO~2~ values.
...
We can get further assistance by proceeding within the chatbot and tell it to create the statistical relationship between CO~2~ and pH and CO~2~ and omega. 
  
You can tell the chatbot to create plots of CO~2~ on the x axis and the other variables on the y axis. It will try to create the plot using python. Please see the instructions in the Supporting Material on learning to plot with python, and learning to plot with python and AI.  
...
> I will provide an IPCC figure with three panels: **var1 (units), var2, and var3 (units)** for multiple SRES scenarios from 2000 to **2XXX**. Visually extract data points from a scenario of my choice every 10 years (2000, **20XX, …, 2XXX**). If axis ticks cannot be read, use default ranges: CO2 **XX–XX**, pH **X.X to X.X** (inverted), Omega **XX–160**. Return: Three CSVs for the chosen scenario (**var1.csv, var2.csv, var3.csv**) and a combined CSV (ipcc_digitized_combined.csv with columns Variable ∈ {**var1,var2,var3**}, Scenario=<chosen>, Year, Value). Plots for **var1, var2, and var3** showing the reconstruction for the chosen scenario. Linear regressions **Year ~ var1**, **Year ~ var2**, and **Year ~ var3** using the chosen scenario, reporting slope, intercept, and R², plus scatter-with-fit plots. If I provide a CSV with columns **Year_model (optional), CO2_uatm, pH, and Omega_pct**, use the regression equations to predict Year from each variable, compare the predictions with the extracted scenario data (variable and year), save a comparison CSV, and include comparison plots. Display all results inline and then provide Python code (pandas, numpy, matplotlib) that reproduces the CSVs and plots from the returned tables.
```
