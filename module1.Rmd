...
```{block, type='rmdnote2'}
Before you begin, open the **Setup** sheet and enter your Student ID in the highlighted cell. This personalises your dataset, so your results are unique to you — always report the numbers from *your* workbook.

Note also that the discharge record contains occasional sensor-gap flags (`-9999`). As with real environmental data, inspect and handle these appropriately before calculating statistics or plotting.

If you feel unfamiliar with Excel, refer to this [getting started page](#excelbasics).
```
...
:::: {.redbox2}
For this module's submission, upload a completed spreadsheet to the LMS location. The file name should follow the pattern:

`STUDENTID_NAME_MODULE01_EnvFlowData.xlsx`

Include in the xlsx file a new sheet called **"RESULTS"**. On that sheet, add your properly formatted graphs and tables summarising the lab. At a minimum this should include:
...
- Q6: Table of the total cumulative amount of diverted water (i.e., the last value) for both diversion scenario options
- Q7 & 8: Scatterplot of the EC vs. daily discharge from the drain, with trendline (hint: exponential), equation and R^2^ value
- Q9: Plot of the daily salinity time-series of the two diversion scenarios compared to the base case
- In 100 words or so, reflect on which is the most appropriate scenario choice for management of the estuary

These are to be included as per the formatting specified in the [Style Guide](#StyleGuide). Marks will be deducted for incorrect formatting.
<!--
...
Professional presentation of results is important in science. Whilst we like Excel as a tool, for those wishing to take their data skills to the next level, explore [this short tutorial](#tidyverse) in the Supporting Material section on how to improve your data viz skills using R's tidyverse package. This is not an assessed component, but note we will use R in future modules.
```