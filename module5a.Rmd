...
As boundary conditions, we have a precipitation dataset, and a function (Equation \@ref(eq:module5a-28)) to simulate potential evaporation (Figure \@ref(fig:plotM5A-1)).
...
Start to write in the equations for storage, using the equations given in the [Equations](#M5A_12) section of this module. Note that there are two variables that have a prime ($\prime$) sign. These variables only exist theoretically in the model, so that we can use them for our IF statements.
...
When you have finished filling in all the columns, see the [Submission](#M5A_21) section for what plots to create and submit.
...
<center>
```{r plotM5A-1, fig.cap="Potential evaporation sin wave function over time.", echo=FALSE, message=FALSE, warning=FALSE, out.width='100%',out.height='100%'}
library(ggplot2)
library(lubridate)
library(plotly)
library(dplyr)
