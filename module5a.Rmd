# Module 5a: Catchment Water Balance {#M5A_1 -}

### Objectives {#M5A_2 -}

To understand a two-store catchment water balance model and implement within Excel or R. Using the supplied parameters and input data, run the model and plot changes in water stores and key hydrological fluxes.

### The Water Balance Approach {#M5A_3 -}
...
<center>
```{r figM5A-1, echo=FALSE, fig.cap="Conceptual model schematic of the 0D ‘bucket’ by Yasmina Elshafei and Matthew Hipsey, adapted from Farmer et al. (2003). Note that $R_g$ and deep groundwater store $S_{gw}$ are depicted here as an optional extension and not included in the exercise.", out.width = '90%'}
knitr::include_graphics("images/module5a/picture1.png")
```
</center>

### Module resources {#M5A_22 -}
...
In Excel, the precipitation is in a column and has a value for every day. The potential evaporation is harder to get hold of. So we use a sine wave function, $sin()$, that takes an annual potl evaporation rate (e.g. 1.5m/yr) and makes a daily potential evaporation value that increases during summer and decrease in winter. Inspect, then copy this formula down the page.
...
library(plotly)
library(dplyr)
evap <- read.csv("plots/module5a/evap.csv")
...
Start to write in the equations for storage, using the equations given in the [Equations](#M5A_12) section of this module. The first is $S_{tot}$ which is the total water in the two stores, $S_{sat}$ and $S_{us}$. 
...
#### 4. Actual evapotranspiration {#M5A_8 -}
...
#### Maximum Storage Capacity of the Bucket/Catchment  {#M5A_13 -}
...
#### Total Evapo-transpiration {#M5A_16 -}
...
#### Agricultural/Shallow Root Crop Evapotranspiration {#M5A_17 -}
...
1) Stacked area plot of water storage over time
2) Plot of $Q_{ie}$ (infiltration excess runoff) and precipitation (on a secondary axis) over time
3) Plot of $Q_{ss}$ (subsurface runoff) and precipitation (on a secondary axis) over time
4) Stacked area plot of $e_{v}$, $e_{bs}$, $e_{ag}$ (i.e., total evaporation) over time

**In less than 100 words write an answer to:**

5) What is the general trend of rainfall over the long term?
6) What is the general trend of storage over the long term? Why does the storage vary over the short term?
7) Which flow mechanism contributes the most to the stream? 
8) What is the dominant ET flux?
