...
In Excel, the date takes the first four columns of the **WATER BALANCE** sheet.
...
Start to write in the equations for storage (**WATER BALANCE** sheet), using the equations given in the [Equations](#M5A_12) section of this module. The first is $S_{tot}$ which is the total water in the two stores, $S_{sat}$ and $S_{us}$. 
...
<center>
\begin{equation}
Q_{se}^t = \left\{\begin{matrix} {S}_{tot}^{t \prime}-\ S_{max},\ \ &{S}_{tot}^{t \prime}>S_{max}
